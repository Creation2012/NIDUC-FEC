...
def tripleBits():
        ...
        csvData.dataToFile(ite, n, len(duze_bledy))
        #print()
...
csvData.clearFile()
if tChoose == 1:
    awg = ko.AWGNChannel(snr=input('Podaj SNR (domyslnie 100.0): '), signal_power=input('Podaj signal_power (domyslnie 1.0): '))
    #awg = ko.AWGNChannel(snr=100.0, signal_power=1.0)
elif tChoose == 2:
    bsc = ko.BinarySymmetricChannel(input('Podaj crossover_probability (domyslnie 0.5): '))
elif tChoose == 3:
    print('Podaj macierz (w kazdym wierszu macierzy suma wartosci musi wynosi 1, do prawdopodobienstwa): ')
    print('x1 x2')
    print('x3 x4')
    dmc = ko.DiscreteMemorylessChannel([input().split(), input().split()])
